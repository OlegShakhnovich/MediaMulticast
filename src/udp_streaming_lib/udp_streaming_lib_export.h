...
typedef enum UdpStreaminLibResult {
    ...
    STREAMING_LIB_ERROR_WRONG_FILE_FORMAT,  /**< Can't parse header of streaming file.
                                        Probably incorrect file type. */
    STREAMING_LIB_ERROR_WRONG_IP_ADDRESS,   /**< IP address can not be parsed. */
    STREAMING_LIB_ERROR_SOCKET_OPEN_FAILED, /**< Socket opening error. */
    STREAMING_LIB_ERROR_INTERNAL_ERROR      /**< Internal library error. */
 ...
 *       The callback is executed in a background thread.
 */
UDP_STREAMING_LIB_API UdpStreaminLibResult startFileStreaming(char* filename, uint32_t filenameLength, char* address,
    uint32_t addrLength, uint16_t port, StreamingCallback callback = NULL, size_t targetBitrate = 0);
